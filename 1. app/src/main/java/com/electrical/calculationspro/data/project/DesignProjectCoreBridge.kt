...
/**
 * Single bridge between the Design Project layer and the
 * Professional Engineering Core facade.
 *
 * Responsibilities:
 * - Project lifecycle
 * - Project validation
 * - Routing engineering requests to ElectricalCalculations
 * - Saving calculation history against the active project
 *
...
object DesignProjectCoreBridge {
    ...
    fun pumpFlowToM3PerSecond(
        input: com.electrical.calculationspro.data.pumps.PumpFlow
    ): Double =
        ElectricalCalculations.pumpFlowToM3PerSecond(
            flow = input
        )
    ...
    fun pumpFlowToM3PerHour(
        ...
        ElectricalCalculations.pumpFlowToM3PerHour(
            flow = input
        )
