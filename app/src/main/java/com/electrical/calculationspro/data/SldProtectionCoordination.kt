...
/**
 * Protection coordination calculation.
 *
 * upstreamEngineering is optional for backward compatibility.
 *
 * When supplied by SldEngineeringFacade, the already calculated
 * upstream engineering result is reused and no second upstream
...
fun calculate(
     ...
     *
     * All protection relationships use the authoritative
     * electrical topology.
     */
    val topology =
    ...
    /*
     * Reuse the shared upstream engineering result when the
     * caller already calculated it.
     *
     * This prevents duplicate engineering calculations during
     * a complete SLD recalculation.
     */
    val upstream =
        upstreamEngineering
    ...
    network.nodes.forEach { node ->
        ...
        val recommendedRating =
            selectBreakerRating(
                downstreamCurrent * BREAKER_MARGIN
            )
        ...
        /*
         * Breaker rating versus calculated design current.
         */
        if (
            recommendedRating <=
            downstreamCurrent
         ...
         *
         * breakerRequiredKa is already the rounded standard
         * interrupting capacity selected by the short-circuit
         * engine. Therefore this check is informational only
         * unless the study contains a valid fault current.
         */
        if (
         ...
         *
         * Incoming feeder is determined from the oriented
         * topology, never from the visual drawing direction.
         */
        if (
    ...
    /*
     * Coordination is evaluated SOURCE -> DOWNSTREAM.
     */
    topology.connections.forEach { connection ->
        ...
        /*
         * Store the calculated margin back into the devices.
         *
         * Data class instances are immutable, so replace the
         * upstream device with an updated copy.
         */
...
private fun determineDownstreamCurrent(
    ...
    /*
     * Primary source:
     * shared upstream engineering result.
     */
    val upstreamCurrent =
        upstream
            .nodes
